<h1 align="center">MeetLink</h1>

<p align="center">
  <strong>轻量级全栈实时在线聊天室系统</strong><br />
  基于 <a href="https://github.com/DWHengr">linyu-mini</a> 二次开发构建
</p>

<p align="center">
  <a href="https://github.com/Cretaceous-pc/MeetLink">GitHub</a> ·
...
| JDK | 21+ | 后端运行环境 |
| Node.js | 20+ | 前端构建运行 |
| Maven | 3.8+ | 后端构建（或使用 `mvnw` 内置 wrapper） |
| 数据库 | MariaDB / PostgreSQL / SQLite | 三选一，SQLite 零配置开箱即用 |

### 方式一：Docker Compose（推荐）

```bash
# 进入部署目录
cd meetlink-server/deploy/compose

# 启动所有服务（MariaDB + Server + Web）
docker compose up -d
...
启动后访问 `http://localhost`，首次启动自动建表并初始化管理员账号。

> 如需 HTTPS，使用 `deploy/compose/https/` 目录下的配置（需自行准备 SSL 证书）。
...
# 方式 A：使用 Maven Wrapper（无需安装 Maven）
...
后端默认使用 **SQLite**，无需安装数据库，首次启动自动建表。
如需切换数据库，修改 `src/main/resources/application.yml` 中的 `spring.datasource.url`：

```yaml
# SQLite（默认，零配置）
url: jdbc:sqlite:meetlink.db

# MariaDB / MySQL
url: jdbc:mariadb://localhost:3306/meetlink?useUnicode=true&characterEncoding=utf-8

# PostgreSQL
url: jdbc:postgresql://localhost:5432/meetlink
```
...
默认连接 `http://localhost:9200`，可在 `.env` 文件中修改 `VITE_HTTP_URL` 和 `VITE_WS_URL`。

### 访问系统

| 地址 | 说明 |
|------|------|
| `http://localhost` | 前端页面（Docker 模式） |
| `http://localhost:5173` | 前端页面（开发模式） |
| `http://localhost:9200` | 后端 REST API |

### 默认管理员

首次启动自动创建管理员账号，邮箱和密码在 `application.yml` 中配置：

```yaml
meetlink:
  admin-email: admin@meetlink.com    # 管理员邮箱
  password: [redacted]                 # 管理员密码（首次登录后请立即修改）
```
...
本项目基于 [linyu-mini](https://github.com/linyu-im/linyu-mini-server) 二次开发，遵循其原始 **Apache License 2.0** 协议开源。
...
本项目基于 **Linyu-Mini** 二次开发，在此向原作者致以诚挚的感谢。

| | |
|---|---|
| **初代作者** | [DWHengr (Heath)](https://github.com/DWHengr) |
| **原始仓库（后端）** | [linyu-im/linyu-mini-server](https://github.com/linyu-im/linyu-mini-server) |
| **原始仓库（前端）** | [linyu-im/linyu-mini-web](https://github.com/linyu-im/linyu-mini-web) |
...
|---|---|
| **开发者** | cheng |
| **站点** | [iocheng.com](https://iocheng.com) |
| **仓库** | [Cretaceous-pc/MeetLink](https://github.com/Cretaceous-pc/MeetLink) |
| **主要改动** | 单聊天室 → 多聊天室、新增管理员、Java 21 + Spring Boot 2.7.18 升级、Vue 3.5 + Vite 6 前端重写、新增 PostgreSQL/SQLite 多数据库支持、WebRTC 文件传输、Docker 部署优化、部分代码细节改进等 |
