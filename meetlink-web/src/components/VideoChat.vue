  ...
  <template v-if="visible">
    ...
    >
      <div class="audio-call" :class="{ reduced: isReduced }">
        ...
        <video ref="local" autoPlay playsinline class="hidden" />
        <video ref="remote" autoPlay playsinline class="hidden" />
      </div>
    </linyu-draggable-window>
    <!--  视频通话-->
...
const handleVideoOfferMsg = async (data) => {
  ...
  await VideoApi.answer({ userId: props.targetInfo.id, desc: pc.value.localDescription }).catch(console.error)
}

const handleICECandidateEvent = (event) => {
  if (event.candidate) {
    VideoApi.candidate({ userId: props.targetInfo.id, candidate: event.candidate }).catch(console.error)
  }
}
...
const onAccept = async () => {
  ...
  VideoApi.accept({ userId: props.targetInfo.id }).catch(console.error)
}
...
const onOffer = async () => {
  ...
  await nextTick(async () => {
    ...
    await VideoApi.offer({ userId: props.targetInfo.id, desc: pc.value.localDescription }).catch(console.error)
  })
}

const onHangup = () => {
  handlerDestroyTime()
  // 保存通话时长，在 visible=false 之前（watch 会重置 time）
  const callDuration = time.value
  // Eager cleanup: stop media tracks immediately
  if (webcamStream.value) {
    webcamStream.value.getTracks().forEach((track) => track.stop())
    webcamStream.value = null
  }
  ...
  VideoApi.hangup({ userId: props.targetInfo.id }).catch(console.error)
  visible.value = false
  MessageApi.send({
    targetId: props.targetInfo.id,
    source: MessageSource.User,
    msgContent: callDuration,
