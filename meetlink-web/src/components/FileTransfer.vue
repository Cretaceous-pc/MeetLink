...
onMounted(() => {
  // EventBus listener registered via watch(visible) when component activates
})
...
const initRTCPeerConnection = () => {
  const iceServer = {
    iceServers: [
      ...
      {
        url: 'turn:link.iocheng.com:3478',
        username: 'meetlink',
        credential: 'yQaN1KFrb2HlHSJu',
      },
    ],
  }
...
const handleICECandidateEvent = (event) => {
  if (event.candidate) {
    FileApi.candidate({ userId: props.targetInfo.id, candidate: event.candidate }).catch(console.error)
  }
}

const handleICEConnectionStateChangeEvent = () => {
  if (pc.value?.iceConnectionState === 'disconnected' || pc.value?.iceConnectionState === 'failed') {
    showToast('传输连接异常~', true)
    visible.value = false
  }
}

const fileAborted = ref(false)

watch(visible, (v) => {
  if (v) {
    EventBus.on('on-receive-file', handlerFileMsg)
    fileAborted.value = false
    receivedChunks.value = []
    receivedSize.value = 0
  } else {
    EventBus.off('on-receive-file', handlerFileMsg)
    fileAborted.value = true
    isReady.value = false
...
const handleFileOfferMsg = async (data) => {
  ...
  await FileApi.answer({ userId: props.targetInfo.id, desc: pc.value.localDescription }).catch(console.error)
}
...
const onAccept = async () => {
  ...
  FileApi.accept({ userId: props.targetInfo.id }).catch(console.error)
}

const onCancel = () => {
  visible.value = false
  if (progress.value < 100) {
    FileApi.cancel({ userId: props.targetInfo.id }).catch(console.error)
  }
}
...
const onOffer = async () => {
  ...
  await nextTick(async () => {
    ...
    await FileApi.offer({ userId: props.targetInfo.id, desc: pc.value.localDescription }).catch(console.error)
  })
}
...
const handleDataChannelMessage = (e) => {
  ...
  if (typeof message === 'object') {
    if (!receivedChunks.value || fileAborted.value) {
